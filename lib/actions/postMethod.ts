...
/**
 * Generic POST method for making authenticated API calls with JSON data.
...
export async function postMethod(
  ...
  try {
      ...
      if (res.status === 401 || res.status === 403) {
        ...
        try {
          ...
          if (refreshResult.success) {
            ...
            console.log("Token refreshed, retrying POST request...");
          ...
          console.error("Token refresh failed during POST retry:", refreshError);
        }
    ...
    // Check if the API returns a standard format with 'success' field
    ...
    // Handle APIs that return code/status format (like company-notes)
 ...
 * @param ticketPublicId - The ticket's public ID
...
export async function addTicketMessage(
  ...
  try {
    const response = await postMethod(
      messageData,
      `/api/v1/auth/web/utility/tickets/${ticketPublicId}/add_message/`
    );

    if (response.success && response.data) {
      return {
        success: true,
        data: response.data as TicketMessage,
    ...
    return {
      success: false,
      message: response.message || "Failed to add message",
